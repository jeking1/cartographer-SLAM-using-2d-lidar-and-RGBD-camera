...
//静态成员函数，全为0：
  static Rigid2<FloatType> Identity() { return Rigid2<FloatType>(); }//返回Rigid2,[0,0,0]


/*为什么要加translation_template：
参见成员模板函数的写法：
...
//归一化角度 ,弧度[-pi;pi]
  double normalized_angle() const {
    return common::NormalizeAngleDifference(rotation().angle()); //方位角θ.以弧度计。
  }
...
*/
  ...
  Rigid2 inverse() const {
    ...
    return Rigid2(translation, rotation);//返回一个新2维刚性变换矩阵.[-dx’,-dy’,-θ]。即上面公式中的R^(-1)和-R^(-1)*t。
  }
...
//参考文章：https://blog.csdn.net/learnmoreonce/article/details/76166192
